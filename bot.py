...
def send_welcome(message):
    text = (
        "🧾 **Умный бот списка покупок и КБЖУ**\n\n"
        "1. Отправляйте товары в формате:\n"
        "`Товар (Магазин) (Цена) (Калории)`\n"
        "**Пример:** `Творог (Чижик) (120) (450 ккал)`\n\n"
        "**Команды:**\n"
        "/shop — открыть текущий список покупок\n"
        "/goal [ккал] — задать цель по калориям (например, `/goal 1800`)\n"
        ...
        "📷 *Отправьте фото чека, чтобы бот обработал покупку!*"
    )
...
def set_calorie_goal(message):
    ...
    if len(parts) < 2 or not parts[1].isdigit():
        bot.reply_to(message, "Укажите цель по калориям числом. Пример: `/goal 1800`", parse_mode="Markdown")
        return
...
def set_budget(message):
    ...
    parts = message.text.split()
    if len(parts) < 2 or not parts[1].isdigit():
        bot.reply_to(message, "Укажите сумму бюджета числом. Пример: `/budget 2500`", parse_mode="Markdown")
        return
...
def generate_menu(message):
    ...
    with get_db() as conn:
        ...
        if not trip or trip['calorie_goal'] <= 0:
            bot.reply_to(message, "⚠️ Сначала задайте цель по калориям командой `/goal [число]` (например, `/goal 2000`) и создайте список через `/shop`.", parse_mode="Markdown")
            return
        
        goal = trip['calorie_goal']
        trip_id = trip['id']
        
        # Сбрасываем статусы всех позиций в 'skipped', а затем активируем оптимальные по калориям
        ...
        # Простой алгоритм подбора под цель (жадный подбор ближайших к цели)
        items = sorted(items, key=lambda x: x['calories'], reverse=True)
        current_sum = 0
        selected_ids = []
        
        for item in items:
            if current_sum + item['calories'] <= goal + 200: # небольшой допуск
...
def show_history(message):
    ...
    for trip in trips:
        ...
        for item in items:
            ...
            text += f"  • {item['name']} ({item['shop']}) — {item['price']} руб.{kcal_str}\n"
...
def generate_trip_content(trip_id):
    ...
    body = "🛒 *РАЦИОН, ЗАТРАТЫ И КБЖУ*\n\n"
    
    body += "📊 *Расходы по магазинам:*\n"
    for shop, total_sum in shop_totals.items():
        body += f"• {shop}: {total_sum} руб.\n"
    body += "\n"
    ...
    for shop, shop_items in shops_dict.items():
        ...
        for item in shop_items:
            ...
            if item['status'] == 'bought':
                icon = "✅"
                line = f"~{item['name']}~ — {item['price']} руб.{kcal_str}"
    ...
    body += f"💰 *Итого куплено:* {grand_total} руб. | 🔥 *Калории:* {grand_calories} ккал"
...
def handle_receipt_photo(message):
    ...
    try:
        ...
        # Сохраняем временно на диск для обработки
        photo_path = f"receipt_{chat_id}.jpg"
        with open(photo_path, 'wb') as new_file:
            new_file.write(downloaded_file)
            
        # Здесь в будущем можно подключить OCR-сервис или нейросеть. 
        # Сейчас бот подтверждает успешную загрузку изображения для чека.
        bot.send_message(chat_id, "✅ Чек успешно загружен и сохранен в системе. (Распознавание позиций по фото готово к интеграции OCR-модуля).")
...
def callback_handler(call):
    ...
    if data.startswith("toggle_"):
        ...
        export_text = "📋 *Мой список покупок и рацион:*\n\n"
        for item in items:
            kcal_str = f" ({item['calories']} ккал)" if item['calories'] > 0 else ""
            export_text += f"• {item['name']} ({item['shop']}) — {item['price']} руб.{kcal_str}\n"
            
        bot.send_message(chat_id, export_text, parse_mode="Markdown")
...
def update_trip_message(message, trip_id):
    ...
    try:
        if len(text) > 4000:
            bot.edit_message_text(chat_id=message.chat.id, message_id=message.message_id, text="⚠️ Список слишком длинный для обновления.", parse_mode="Markdown")
...
def parse_items(message):
    ...
    if added > 0:
        ...
        bot.reply_to(message, "Не удалось распознать позиции. Формат: `Товар (Магазин) (Цена) (Калории)`", parse_mode="Markdown")
